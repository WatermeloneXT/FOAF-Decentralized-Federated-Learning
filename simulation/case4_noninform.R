...
beta_result2 <- decentralized_gradient_descent(X_list, y_list, graph = graph  ,
                                               ...
                                               rate = 0.5, neighbor_num = 2)

beta_result3 <- decentralized_gradient_descent(X_list, y_list, graph = graph + diag(K),
                                               epochs = 1e5, lr = 0.01,
...
saveRDS(data_list, "linear_subhomo_case4_noninform.rds")

#data_list = readRDS("linear_subhomo_case4_noninform.rds")
...
est_avg2 = est_avg2[1:6e4]
est_std2 = est_std2[1:6e4]
...
df_all <- bind_rows(
  ...
  data.frame(Epoch = epochs, Method = "FOAF", Value = beta_result2$rmse[1:6e4], Measure = "RMSE"),
...
p_rmse <- ggplot(df_rmse, aes(x = Epoch, y = Value, color = Method)) +
  geom_line(size = 1) +
  labs(title = "Non-Informative Communication Topology", x = "epochs", y = "rmse") +
  scale_x_continuous(limits = c(0, 35000),labels = function(x) x / 100) +
  scale_color_manual(values = prestige_colors) + ylim(0,6)+
  theme_minimal(base_size = 10) +
  theme(legend.position = "none")
