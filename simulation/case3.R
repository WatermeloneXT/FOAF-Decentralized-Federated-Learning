decentralized_gradient_descent <- function(X_list, y_list, graph,
  ...
  for (t in 1:epochs) {
    ...
    beta_avg_list[[t]] <- mean(rowMeans(  abs(beta_mat - true_beta) ))
...
beta_result1 <- decentralized_gradient_descent(X_list, y_list, graph = graph ,
                                               epochs = 1e5, lr = 0.1, true_beta = true_beta,
...
#saveRDS(data_list, "linear_totalheter_case1.rds")
